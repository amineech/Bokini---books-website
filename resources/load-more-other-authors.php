<?php 
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        $chosen_author = $_POST['chosen_author'];

        // get data from database
        $sql = $mysql->prepare('select * from author where idAuthor > ? and idAuthor not in(?) order by idAuthor asc limit 5');
        $sql->bind_param('ii', $last_idAuthor, $chosen_author);
        $sql->execute();
        ...
        if(mysqli_num_rows($authors) > 0){
            ...
            $load = '<div class="load-more-auth2">
                        <!-- the input contain the id of the last displayed author -->
                        <button class="load-more-btn">Load more</button>
                        <input type="hidden" value="' . $chosen_author .'" class="val_chosenAuthor">
